...
def make_post(item: dict) -> str:
    ...
    prompt = f"""Режим «Посты». Напиши пост для канала IPM | LAB по этой новости — от лица Константина Пороцкого, с его развёрнутым личным экспертным комментарием.

Заголовок новости: {item['title']}
Краткое описание: {item['summary']}

Как писать:
- Тип поста — ЭКСПЕРТНЫЙ, с насыщенным авторским комментарием Константина. Это не пересказ новости, а его глубокий взгляд на неё.
- Дай БОЛЬШЕ содержания и пользы: разбери, что это значит для застройщика/инвестора, какие плюсы и риски, на что смотреть, к чему это ведёт на рынке КРТ. Несколько разных углов, а не одна мысль.
- Структура: цепляющий заголовок → зачин → 3–4 содержательные мысли через опыт девелопера → финал-вопрос к аудитории.
...
- Объём — насколько можно содержательно, но не длиннее 800 знаков (пост идёт подписью к картинке). Плотно, без воды.
