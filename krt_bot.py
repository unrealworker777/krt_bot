...
SOURCES = [
    ...
    #    это базы/сервисы, их нужно мониторить отдельными парсерами):
    #    ГИС Торги (torgi.gov.ru), Росэлторг, НСПД/кадастр (nspd.gov.ru),
    #    Росреестр, publication.pravo.gov.ru, regulation.gov.ru, sozd.duma.gov.ru,
    #    КАД (kad.arbitr.ru), Судакт, ВС РФ, КонсультантПлюс.
]
...
Как писать:
- Официально-деловой тон: нейтрально, точно, профессионально. Без эмоций и риторических вопросов.
- СТРОГО БЕЗ ПЕРВОГО ЛИЦА: не используй слова «я», «мне», «мой», «мы», «наш», «на мой взгляд», «считаю». Никакого личного авторства и личных историй. Пиши безлично и от третьего лица (например: «закон устанавливает…», «для застройщиков это означает…», «изменения затрагивают…»). Без обращений к читателю на «ты»/«вы».
- Структура: заголовок → суть новости (что произошло) → деловой разбор, что это значит для застройщиков/инвесторов в контексте КРТ или ИЖС (последствия, требования, сроки, риски) → сдержанный итог.
...
- Объём — до 800 знаков. Плотно, по существу, без воды.
