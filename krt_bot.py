...
def _clean_emoji(text: str) -> str:
    ...
    keep = {"📍", "🇷", "🇺"}          # 🇷🇺 состоит из двух символов-индикаторов
    out = []
    for ch in (text or ""):
        code = ord(ch)
        is_emoji = (
...
ЗАДАЧА 2 — ТЕЗИСНЫЙ РАЗБОР:
Проанализируй текст статьи и выпиши ТЕЗИСНО, по существу, что в ней главного: конкретные факты, цифры, сроки, требования, решения. Каждый тезис — отдельной строкой, начинается с «— ». 3–5 тезисов. Тезисы должны нести реальную информацию из статьи, а не общие слова. Затем 1–2 предложения делового вывода: что это значит для застройщиков/инвесторов в контексте КРТ или ИЖС.

ТРЕБОВАНИЯ К СТИЛЮ:
- Официально-деловой тон: нейтрально, точно, профессионально. Без восклицаний и риторических вопросов.
- СТРОГО БЕЗ ПЕРВОГО ЛИЦА: не используй «я», «мне», «мой», «мы», «наш», «считаю». Пиши безлично и от третьего лица. Без обращений на «ты»/«вы».
- Без вводных фраз-воды («Как известно», «Стоит отметить», «В последнее время»).
- ЭМОДЗИ: только 🇷🇺 — и только если речь о федеральном уровне (Минстрой, Госдума, Правительство РФ, ДОМ.РФ, федеральные законы). НИКАКИХ других эмодзи в тексте. Не используй 🔹, 🔷, ✅ и любые прочие значки.
...
- Объём — до 850 знаков. Плотно, без воды.
...
🇷🇺 Правительство утвердило поправки, которые меняют порядок работы с территориями комплексного развития.

— Срок рассмотрения документации сокращён с 30 до 15 рабочих дней.
— Решение о КРТ теперь может приниматься без публичных слушаний для промзон.
— Требования к социальной инфраструктуре закреплены на этапе договора.

Для застройщиков это сокращает предпроектную стадию и снижает риск переноса сроков ввода. Проекты в промышленных зонах становятся предсказуемее по срокам входа.
...
Верни ТОЛЬКО текст поста, без пояснений."""
    ...
    body = _clean_emoji(body)               # оставляем только 📍 и 🇷🇺
...
def publish_approved():
    ...
    for mid in list(approved_ids):
        ...
        try:
            ...
            cover = make_cover(title, data.get("source", ""), "design",
                               data.get("image_url") or None)
            send_photo(TELEGRAM_CHANNEL, cover, ensure_footer(data.get("text", "")))
