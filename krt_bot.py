...
# Один и тот же 4-абзацный шаблон в каждом посте делает ленту однообразной.
# Держим набор форматов и чередуем их: меняется и структура, и объём.
...
POST_FORMATS = [
    {
        ...
        "limit": 850,
        "task": (
            "ТЕЗИСНЫЙ РАЗБОР. Выпиши 3–5 тезисов по существу — конкретные факты, "
            "цифры, сроки, требования, решения. Каждый тезис отдельной строкой, "
            "начинается с «— ». Затем 1–2 предложения делового вывода."
        ),
        "shape": (
            "<b>Регионы получили право сокращать сроки согласования проектов КРТ</b>\n\n"
            "🇷🇺 Правительство утвердило поправки к порядку работы с территориями "
            "комплексного развития.\n\n"
            "— Срок рассмотрения документации сокращён с 30 до 15 рабочих дней.\n"
            "— Решение о КРТ для промзон принимается без публичных слушаний.\n"
            "— Требования к социальной инфраструктуре закреплены на этапе договора.\n\n"
            "Предпроектная стадия сокращается, риск переноса сроков ввода снижается."
        ),
    },
    {
        "id": "brief",
        "limit": 420,
        "task": (
            "КОРОТКАЯ ЗАМЕТКА. Только суть: 2–3 предложения сплошным текстом, без "
            "списков и тезисов. Первое предложение — само событие с конкретикой, "
            "второе-третье — что оно меняет на практике."
        ),
        "shape": (
            ...
            "проектам КРТ в 15 рабочих дней вместо прежних 30. Норма распространяется "
            "на все решения, принятые после 1 сентября. Предпроектная стадия "
            "сокращается примерно на месяц."
        ),
    },
    {
        "id": "what_changed",
        "limit": 950,
        "task": (
            "РАЗБОР ПО СХЕМЕ. Три коротких смысловых блока, каждый с подзаголовком "
            "в <b>жирном</b> на отдельной строке: «Что изменилось», «Кого касается», "
            "«Что делать». Под каждым — 1–2 предложения или 2 тезиса с «— »."
        ),
        "shape": (
            "<b>Публичные слушания по промзонам отменены</b>\n\n"
            "<b>Что изменилось</b>\n"
            "Решение о КРТ нежилой застройки принимается без публичных слушаний.\n\n"
            "<b>Кого касается</b>\n"
            "Застройщиков промышленных площадок и правообладателей смежных участков.\n\n"
            "<b>Что делать</b>\n"
            "— Пересчитать график входа в проект: минус этап согласования.\n"
            "— Отслеживать публикацию проектов решений на портале региона."
        ),
    },
    {
        "id": "figure_lead",
        "limit": 700,
        "task": (
            "ЛИД С ЦИФРОЙ. Начни основной текст с самой значимой цифры из статьи — "
            "она должна стоять в первом предложении и быть выделена <b>жирным</b>. "
            "Дальше 2 коротких абзаца: откуда цифра и что она означает для рынка. "
            "Без списков."
        ),
        "shape": (
            "<b>Под КРТ в регионе отведено 340 гектаров</b>\n\n"
            "<b>340 га</b> — площадь территорий, включённых в перечень комплексного "
            "развития по итогам пересмотра границ.\n\n"
            "Перечень утверждён по результатам инвентаризации промышленных площадок. "
            "Основной объём приходится на три бывшие производственные зоны.\n\n"
            "Для девелоперов это расширяет пул площадок с подготовленным "
            "градостроительным статусом."
        ),
    },
    {
        "id": "qa",
        "limit": 800,
        "task": (
            "ВОПРОС — ОТВЕТ. Сформулируй practical-вопрос, который возникает у "
            "участника рынка в связи с новостью, поставь его отдельной строкой "
            "<i>курсивом</i>, затем дай ответ по фактам статьи в 2–4 предложениях "
            "или 2–3 тезисах с «— »."
        ),
        "shape": (
            "<b>Изъятие участков в границах КРТ: порядок уточнён</b>\n\n"
            "Порядок изъятия недвижимости в границах КРТ дополнен требованиями к "
            "оценке.\n\n"
            "<i>Что это меняет для правообладателя внутри границ проекта?</i>\n\n"
            "— Оценка проводится на дату принятия решения о КРТ, а не на дату изъятия.\n"
            "— Срок направления проекта соглашения — 30 дней с момента оценки."
        ),
    },
]
...
ЗАДАЧА 3 — ВЗГЛЯД ИПМ:
Заверши пост отдельным блоком: строка «<b>Взгляд ИПМ:</b>», а под ней 1–2 строки — что конкретно новость означает для девелопера. Не общая фраза, а практический вывод: на что повлияет в сроках, деньгах, документах или структуре сделки. Если фактуры в статье мало — напиши, на что стоит обратить внимание при отслеживании темы, но без выдуманных деталей.{question_task}
...
- Объём — до {fmt['limit']} знаков вместе с блоком «Взгляд ИПМ». Плотно, без воды.
...
<b>Взгляд ИПМ:</b>
Вход в проекты по промзонам ускоряется на 3–4 недели. Графики финансирования, привязанные к дате согласования, стоит пересчитать.
...
- Заверши блоком: строка «<b>Взгляд ИПМ:</b>» и под ней 1–2 строки практического вывода для девелопера.
...
- Заверши блоком: строка «<b>Взгляд ИПМ:</b>» и под ней 1–2 строки — где на этом пути чаще всего теряют время или деньги.
...
- Заверши блоком «<b>Взгляд ИПМ:</b>» и 1–2 строками: при каких вводных площадка интересна, при каких — нет.
